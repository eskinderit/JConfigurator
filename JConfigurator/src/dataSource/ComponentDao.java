...
public abstract class ComponentDao<T1 extends Component, T2 extends ComponentDao<T1, T2>> {
	...
	protected ArrayList<T1> _removeComponents(ArrayList<T1> toDeleteList, String myFile, Class<T2> class2Bound)
	...
	protected ArrayList<T1> _addComponents(String myFile, ArrayList<T1> componentListToAdd, Class<T2> class2Bound)
			throws JAXBException {
		...
		if (componentDao.getComponentList() != null) {
			...
			// Scorre le liste per vedere se un elemento che vogliamo aggiungere è già
			// presente
			...
			for (T1 component : componentListToAdd) {
				...
				if (i == 0)
					newComponentList.add(component); // Aggiunge SOLO se NON è stato incontrato nessun elemento uguale a
														// component
			}
	...
	protected ArrayList<T1> _setDefaultComponents(String sourceFile, String destinationFile, Class<T2> class2Bound)
			throws JAXBException {
